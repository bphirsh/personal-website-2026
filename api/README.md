# Workspace beta — backend

Two Vercel serverless functions that power `/workspace/beta-signup/` and `/workspace/beta-download/`. They deploy with `git push`, exactly like the rest of the site.

| Endpoint | Does |
|---|---|
| `POST /api/signup` | Validates a name + email-or-phone, emails it to you |
| `POST /api/beta-auth` | Checks the beta password, returns the build's URL |
...
- **No database.** Signups are emailed, not stored. For a beta list this size an inbox is a perfectly good record, and it removes a whole service from the stack.
...
- **The build is not streamed through a function.** At ~119MB that risks the function timeout on a slow connection and bills the bandwidth twice (Blob → function → browser). `beta-auth` hands back Blob's CDN URL instead and the browser downloads directly.
...
|---|---|---|
| `BETA_PASSWORD` | The shared beta password | yes |
...
```bash
curl -s -X POST https://www.brianhirsh.com/api/beta-auth \
...
- **One shared password.** Anyone who has it can pass it on, and you can't revoke one person. Fine for a small beta.
- **Vercel Hobby is non-commercial.** A free friends-and-family beta is almost certainly fine. If Workspace becomes something you charge for while still distributing here, that's Pro territory.

## Note on Gatekeeper

The install text tells testers to right-click → **Open**. On macOS 15 (Sequoia) and later that path was removed for unsigned apps — they now have to go to **System Settings → Privacy & Security** and click **Open Anyway** after the first blocked launch. If your testers are on Sequoia or newer, either update that copy or notarise the build.
